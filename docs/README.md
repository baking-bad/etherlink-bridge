> [!NOTE]
> This document is a conceptual walkthrough of the bridging process. The contract
> addresses, byte payloads, and explorer links in the examples are **illustrative**
> — substitute your own deployment's values. Commands run through the `bridge` CLI;
> select the network with `NETWORK=` (see [Selecting a network](../README.md#selecting-a-network)).
...
```shell
uv run bridge deploy_token \
    --token-type FA1.2 \
    --total-supply 1000
```
Here is an example of the deployed [token](https://shadownet.tzkt.io/KT19P1nbGzGnumMfRHcLNuyQUdcuwjpBfsCU/operations/).
...
```
Here is an example of the deployed [ticketer](https://shadownet.tzkt.io/KT1KjXCapuLqYjHjV6Ce7Mfsm7uiqRZY3Vw8/metadata/).
...
```shell
uv run bridge get_ticketer_params \
...
uv run bridge deploy_erc20 \
    ...
    --token-decimals 0
```
Here is an example of the deployed [token](https://shadownet.explorer.etherlink.com/address/0x03E39FF2b379FBcd9284Ab457113D82fF4daBBF4).
...
```
Here are examples of these operations on the [Tezos](https://shadownet.tzkt.io/onwj8usqmb1snvh35jzzsvxgM7U5rKazpz78pCEws12VLAaY8GW/16767) and [Etherlink](https://shadownet.explorer.etherlink.com/tx/0x29a281a68f50cc43e0c1af17f2e492d266c232d100403eca722d89c80c860d81) sides.
...
To demonstrate this operation, the `fa_withdraw` command is provided:
```shell
...
Here is an example of the [withdraw](https://shadownet.explorer.etherlink.com/tx/0xac586320f475653b5fe8c4e0ce40f61147fd556069d0c268a989c98e8a31c3c1) operation.
...
```shell
uv run bridge get_proof --level 1181470 --index 0
```
...
```shell
uv run bridge execute_outbox_message \
...
```
Here is an example of the finished [withdrawal](https://shadownet.tzkt.io/oo9FJdy6byfy6HoTqpnzs68eLrpfwu1aouvnM8bv6HCbjrsXPF2/228622).